...
-- A How many unique nodes are on the Data Bank system?
SELECT
...
SELECT
r.region_name,COUNT(DISTINCT cn.node_id)   AS node_count
...
SELECT
r.region_name,COUNT(DISTINCT cn.customer_id)   AS customer_count
FROM customer_nodes cn
JOIN regions r
    ON cn.region_id = r.region_id
GROUP BY r.region_name
...
SELECT region_name,
    ROUND(PERCENTILE_CONT(0.50) WITHIN GROUP
        (ORDER BY days), 0)   AS median_days,
    ROUND(PERCENTILE_CONT(0.80) WITHIN GROUP
        (ORDER BY days), 0)   AS p80_days,
    ROUND(PERCENTILE_CONT(0.95) WITHIN GROUP
        (ORDER BY days), 0)   AS p95_days
FROM node_days
GROUP BY region_name ORDER BY region_name;


-- B Unique count and total amount for each transaction type
SELECT
    txn_type,
    COUNT(*)  AS txn_count,
...
-- Average total historical deposit count and amounts for all customers
SELECT
    ROUND(AVG(dep_count),  2)   AS avg_deposit_count,
        ROUND(AVG(dep_amount), 2)   AS avg_deposit_amount
FROM
	(SELECT
        customer_id,
        COUNT(*)         AS dep_count,
...
-- Customers with >1 deposit AND (≥1 purchase OR ≥1 withdrawal) per month
SELECT month, COUNT(DISTINCT customer_id) AS qualifying_customers
FROM (
    SELECT
        customer_id,
...
FROM monthly_net ORDER BY customer_id, month;

-- Percentage of customers who increased closing balance by more than 5%
WITH closing AS (
    SELECT customer_id, month, closing_balance,
           ROW_NUMBER() OVER (PARTITION BY customer_id ORDER BY month)      AS rn_asc,
           ROW_NUMBER() OVER (PARTITION BY customer_id ORDER BY month DESC) AS rn_desc
    FROM monthly_closing   -- from B4 CTE
),
first_last AS (
    SELECT customer_id,
           MAX(CASE WHEN rn_asc  = 1 THEN closing_balance END) AS first_bal,
           MAX(CASE WHEN rn_desc = 1 THEN closing_balance END) AS last_bal
    FROM closing GROUP BY customer_id)
SELECT ROUND(
    100.0 * SUM(last_bal > first_bal * 1.05) / COUNT(*),
2) AS pct_increased_over_5pct
FROM first_last WHERE first_bal > 0;


-- Data allocation — running balance, month-end balance, min/avg/max
-- Option 1: End-of-month balance (closing_balance from B4)
SELECT month, SUM(GREATEST(closing_balance, 0))  AS data_option1
FROM monthly_closing GROUP BY month;
-- Option 2: Avg balance in previous 30 days
SELECT month,
    SUM(GREATEST(avg_running_balance, 0))  AS data_option2
FROM (
    SELECT customer_id,
        DATE_FORMAT(txn_date,'%Y-%m')  AS month,
...
-- Interest-based data growth — 6% APR, simple and compound daily
-- Daily rate = 6 / 365 / 100 = 0.000164384
WITH running AS (
    SELECT customer_id,
           txn_date,
           DATE_FORMAT(txn_date,'%Y-%m')    AS month,
...
SELECT month,
    -- Simple interest
    ROUND(SUM(CASE WHEN balance > 0
                   THEN balance * 0.06 / 365
				ELSE 0 END),2 ) AS additional_data_unit ,
    -- Compound: balance*(1+r)^1 - balance per day
    ROUND(SUM(CASE WHEN balance > 0
                   THEN balance * (POW(1 + 0.06/365, 1) - 1)
                   ELSE 0 END), 2)  AS compound_interest_data
FROM running GROUP BY month ORDER BY month;
